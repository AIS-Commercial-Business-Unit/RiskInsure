...
export function getFileProcessingConfig(): FileProcessingConfig {
  return {
    apiBaseUrl: process.env.FILE_PROCESSING_API_BASE_URL || 'http://127.0.0.1:7090',

    jwtSecret: process.env.FILE_PROCESSING_JWT_SECRET || 'REPLACE_WITH_SECRET_KEY_AT_LEAST_32_CHARS_LONG_FOR_PRODUCTION',
    jwtIssuer: process.env.FILE_PROCESSING_JWT_ISSUER || 'https://riskinsure.com',
    jwtAudience: process.env.FILE_PROCESSING_JWT_AUDIENCE || 'file-processing-api',
    bearerToken: process.env.FILE_PROCESSING_BEARER_TOKEN,

    ftpContainerName: process.env.FILE_PROCESSING_FTP_CONTAINER || 'file-processing-ftp',
    ftpPassword: process.env.FILE_PROCESSING_FTP_PASSWORD_SECRET_NAME || 'testpass',
    ftpHost: process.env.FILE_PROCESSING_FTP_HOST || 'file-processing-ftp',
    ftpPort: parseInt(process.env.FILE_PROCESSING_FTP_PORT || '21', 10),
    ftpUsername: process.env.FILE_PROCESSING_FTP_USERNAME || 'testuser',

    httpsContainerName: process.env.FILE_PROCESSING_HTTPS_CONTAINER || 'file-processing-https',
    // The Host URLs can be used by something running on the host system
    httpHostBaseUrl: process.env.FILE_PROCESSING_HTTP_BASE_URL || 'http://127.0.0.1:8080',
    httpsHostBaseUrl: process.env.FILE_PROCESSING_HTTPS_BASE_URL || 'https://127.0.0.1:8443',
    // The Container URLs can be used by something running in a container
    httpContainerBaseUrl: process.env.FILE_PROCESSING_HTTP_BASE_URL || 'http://file-processing-https:80',
    httpsContainerBaseUrl: process.env.FILE_PROCESSING_HTTPS_BASE_URL || 'https://file-processing-https:443',

    azuriteContainerName: process.env.FILE_PROCESSING_AZURITE_CONTAINER || 'file-processing-azurite',
    azuriteStorageAccountName: process.env.FILE_PROCESSING_AZURITE_STORAGE_ACCOUNT || 'devstoreaccount1',
    azuriteBlobContainerName: process.env.FILE_PROCESSING_AZURITE_BLOB_CONTAINER || 'e2e-files',
    azuriteHostBlobConnectionString:
      process.env.FILE_PROCESSING_AZURITE_HOST_CONNECTION_STRING ||
      "DefaultEndpointsProtocol=http;AccountName=devstoreaccount1;AccountKey=Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq/K1SZFPTOtr/KBHBeksoGMGw==;BlobEndpoint=http://localhost:10000/devstoreaccount1;",
    azuriteContainerBlobConnectionString:
      process.env.FILE_PROCESSING_AZURITE_CONTAINER_CONNECTION_STRING ||
      "DefaultEndpointsProtocol=http;AccountName=devstoreaccount1;AccountKey=Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq/K1SZFPTOtr/KBHBeksoGMGw==;BlobEndpoint=http://file-processing-azurite:10000/devstoreaccount1;",
  };
}
...
export async function seedFileToHttpsContainer(fileName: string, fileContent: string): Promise<string> {
  const testDataDir = await resolveFileProcessingTestDataDir();
...
async function resolveFileProcessingTestDataDir(): Promise<string> {
  const currentFilePath = fileURLToPath(import.meta.url);
  const startDir = path.dirname(currentFilePath);

  let currentDir = startDir;
  for (let i = 0; i < 8; i += 1) {
    const candidate = path.join(currentDir, 'platform', 'fileprocessing', 'test-data');
    try {
  ...
  throw new Error('Could not locate platform/fileprocessing/test-data directory from test/e2e/helpers.');
}
