# Data Model: Client File Processing Configuration
...
**Relationships**:
- One-to-Many with `FileProcessingExecution` (one configuration → many executions)
- One-to-Many with `DiscoveredFile` (one configuration → many discovered files)

**Storage**:
- Cosmos DB container: `file-processing-configurations`
- Partition key: `/clientId`
- Document type discriminator: `FileProcessingConfiguration`
...
### 2. FileProcessingExecution

**Purpose**: Represents a single execution attempt of a FileProcessingConfiguration, tracking success/failure and discovered files.
...
**Relationships**:
- Many-to-One with `FileProcessingConfiguration`
- One-to-Many with `DiscoveredFile` (one execution → many files discovered)

**Storage**:
- Cosmos DB container: `file-processing-executions`
- Partition key: `/clientId/configurationId` (hierarchical)
- Document type discriminator: `FileProcessingExecution`
...
**Relationships**:
- Many-to-One with `FileProcessingConfiguration`
- Many-to-One with `FileProcessingExecution`
...
**Purpose**: Defines when a FileProcessingConfiguration should execute.
...
```
FileProcessingConfiguration (1) ----< (M) FileProcessingExecution
                                         |
                                         +----< (M) DiscoveredFile

FileProcessingConfiguration (1) ----< (M) DiscoveredFile (direct)
```
...
1. **FileProcessingConfiguration Creation**:
...
### FileProcessingExecution State Transitions
...
### FileProcessingConfiguration Document Example

```json
{
  "id": "e3a85f64-5717-4562-b3fc-2c963f66afa6",
  "clientId": "client-acme-corp",
  "documentType": "FileProcessingConfiguration",
...
### FileProcessingExecution Document Example
...
```json
{
  ...
  "documentType": "FileProcessingExecution",
...
1. **Optimistic Concurrency**: All updates to FileProcessingConfiguration use ETag checks (prevent lost updates)
...
5. **Immutable History**: FileProcessingExecution and DiscoveredFile are append-only (never updated after terminal state)
...
2. **Document Size**:
   - FileProcessingConfiguration: Typically < 10 KB (small, metadata only)
   - FileProcessingExecution: < 5 KB per execution (metadata only)
...
4. **TTL (Time-To-Live)**:
   - `file-processing-executions`: 90 days (spec assumption)
...
✅ **Optimistic concurrency** for safe concurrent updates (FileProcessingConfiguration)  
