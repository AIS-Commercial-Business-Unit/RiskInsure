# Research: Client File Processing Configuration
...
This document captures research findings, architectural decisions, and technology choices for the Client File Processing Configuration feature. Each section addresses key unknowns identified in the Technical Context (marked as "NEEDS CLARIFICATION") and provides rationale for decisions made.
...
- ✅ No additional persistence required (schedules stored in FileProcessingConfiguration)
...
```
FileProcessing.Worker (Container App)
  ↓
...
1. **SchedulerHostedService** runs as a `BackgroundService` in `FileProcessing.Worker`
...
### Research Question
Should FileProcessingConfiguration be partitioned by `clientId` alone or use hierarchical key `/clientId/configurationId`?
...
|-----------|---------------|-----------|
| `file-processing-configurations` | `/clientId` | Client-scoped queries, bounded per client |
| `file-processing-executions` | `/clientId/configurationId` (hierarchical) | Execution history per configuration, high volume |
...
- Include `User-Agent` header (identify as "RiskInsure File Processing")
...
### Research Question
What logging and error handling patterns should we use for file processing operations?
