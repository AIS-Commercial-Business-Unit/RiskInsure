...
- Azure Cosmos DB SDK (FileProcessingConfiguration storage, execution history)
...
**Storage**: 
- Azure Cosmos DB (FileProcessingConfiguration, FileProcessingExecution history, DiscoveredFile tracking)
- Partition strategy: NEEDS CLARIFICATION (by clientId for multi-tenant isolation, or hierarchical /clientId/configId)
- Document types: FileProcessingConfiguration, FileProcessingExecution, DiscoveredFile
...
- REST API for CRUD operations on FileProcessingConfiguration

**Project Type**: Platform service (new bounded context: "File Processing")
...
**Performance Goals**:
- Support 100+ FileProcessingConfigurations per client without degradation
...
**Scale/Scope**:
- Support 1,000+ FileProcessingConfigurations across all clients
...
**Status**: ✅ **PASS**  
**Assessment**: File processing configurations will use single-partition model in Cosmos DB.  
**Design**:
- One container for file processing documents
...
- `file-processing-configurations`: Partition key `/clientId`
- `file-processing-executions`: Partition key `/clientId/configurationId` (hierarchical)
...
**Design**:
- All FileProcessingConfiguration updates include ETag checks
...
- ETag fields on all mutable entities (FileProcessingConfiguration)
- State transition matrices for FileProcessingExecution and DiscoveredFile
...
**Status**: ✅ **PASS**  
**Assessment**: Comprehensive observability required for troubleshooting file processing issues (User Story 6).  
**Design**:
...
```text
specs/001-file-processing-config/
...
```text
services/
├── file-processing/                        # NEW bounded context for this feature
│   ├── FileProcessing.Domain/              # Domain layer (entities, value objects)
│   │   ├── Entities/
│   │   │   ├── FileProcessingConfiguration.cs
│   │   │   ├── FileProcessingExecution.cs
│   │   │   └── DiscoveredFile.cs
│   │   ├── ValueObjects/
...
│   │       ├── IFileProcessingConfigurationRepository.cs
│   │       ├── IFileProcessingExecutionRepository.cs
...
│   │
│   ├── FileProcessing.Infrastructure/        # Infrastructure layer (data access, external services)
│   │   ├── Repositories/                    # Cosmos DB implementations
│   │   │   ├── FileProcessingConfigurationRepository.cs
│   │   │   ├── FileProcessingExecutionRepository.cs
...
│   │
│   ├── FileProcessing.API/                   # REST API for configuration management
│   │   ├── Controllers/
...
│   │
│   ├── FileProcessing.Worker/                # Background worker for scheduled checks
...
│   │
│   └── docs/
│       └── file-processing-standards.md      # Domain terminology and standards
│
└── workflow/                                # EXISTING workflow orchestration platform
    └── (integration point via Service Bus messages)

tests/
├── FileProcessing.Domain.Tests/              # Domain unit tests (90%+ coverage)
│   ├── Entities/
│   ├── ValueObjects/
│   └── TokenReplacementTests.cs             # Comprehensive token logic tests
│
├── FileProcessing.Application.Tests/         # Application unit tests (80%+ coverage)
│   ├── Services/
│   ├── Protocols/
...
│   └── MessageHandlers/
│
└── FileProcessing.Integration.Tests/         # Integration tests
...
**Structure Decision**: New bounded context "File Processing" as a separate service under `services/file-processing/`. This aligns with the existing `services/workflow/` structure from the workflow orchestration platform. The feature is decomposed into standard layers:
