# Quickstart Guide: Client File Processing Configuration
...
This guide helps developers quickly understand, build, and test the Client File Processing Configuration feature. It provides step-by-step instructions for local development, testing, and integration with the workflow orchestration platform.

---

## Architecture Summary

The File Processing feature is a **new bounded context** that:
...
**Key Components**:
1. **FileProcessing.API** - REST API for CRUD operations on configurations
2. **FileProcessing.Worker** - Background service that executes scheduled file checks
3. **FileProcessing.Domain** - Business entities and rules
4. **FileProcessing.Application** - Services, protocol adapters, message handlers
5. **FileProcessing.Infrastructure** - Cosmos DB repositories, scheduling
...
git checkout 001-file-processing-config
```
...
**FileProcessing.API/appsettings.Development.json**:
```json
{
  "CosmosDb": {
...
**FileProcessing.Worker/appsettings.Development.json**: Same as above
...
This creates:
- Container: `file-processing-configurations` (partition key: `/clientId`)
- Container: `file-processing-executions` (partition key: `/clientId/configurationId`)
...
```bash
cd tests/FileProcessing.Domain.Tests
dotnet test

cd ../FileProcessing.Application.Tests
dotnet test
```
...
```bash
cd tests/FileProcessing.Integration.Tests
dotnet test
```
...
- Browse containers: `file-processing-configurations`, `file-processing-executions`, `discovered-files`
...
```bash
git checkout -b feature/file-processing-enhancements
```
...
git push origin feature/file-processing-enhancements
```
...
docker-compose up -d file-processing-https
...
4. **Production Deployment**:
   - Follow deployment guide: [docs/deployment.md](../../../services/file-processing/docs/deployment.md)
...
- **Slack**: #file-processing-dev
- **Email**: dev-team@riskinsure.com
- **Docs**: [Internal Wiki](https://wiki.riskinsure.com/file-processing)
