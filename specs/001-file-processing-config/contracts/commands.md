# Command Contracts: Client File Processing Configuration
...
- Handler checks for existing `FileProcessingExecution` with same key before processing
...
**Purpose**: Creates a new FileProcessingConfiguration.
...
**Purpose**: Updates an existing FileProcessingConfiguration.
...
**Purpose**: Deletes (soft-delete by setting IsActive = false) a FileProcessingConfiguration.
...
**Notes**:
- This command is defined in `FileProcessing.Contracts` but handled by workflow platform
- Workflow platform subscribes to this command type via NServiceBus routing
- CommandData comes from CommandDefinition in FileProcessingConfiguration (static metadata)
...
**FileProcessing.Worker** (sends commands):
```csharp
...
```csharp
var routing = endpointConfiguration.UseTransport<AzureServiceBusTransport>().Routing();
routing.RouteToEndpoint(typeof(CreateConfiguration), "FileProcessing.Worker");
routing.RouteToEndpoint(typeof(UpdateConfiguration), "FileProcessing.Worker");
routing.RouteToEndpoint(typeof(DeleteConfiguration), "FileProcessing.Worker");
```
