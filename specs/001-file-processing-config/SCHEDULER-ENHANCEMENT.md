...
**I want** to configure how frequently the scheduler checks for due file processing configurations  
...
**New Files:**
- `FileProcessing.Infrastructure/Configuration/SchedulerOptions.cs` - Configuration class with validation
- `services/file-processing/docs/SCHEDULER-CONFIGURATION.md` - Complete documentation

**Modified Files:**
- `FileProcessing.Infrastructure/Scheduling/SchedulerHostedService.cs` - Now uses IOptions<SchedulerOptions>
- `FileProcessing.Infrastructure/DependencyInjection.cs` - Registers SchedulerOptions and ScheduleEvaluator
- `FileProcessing.Worker/appsettings.json` - Added ExecutionWindowMinutes setting
...
SchedulerHostedService (Timer: PollingIntervalSeconds)
  ↓
Query Cosmos DB (IFileProcessingConfigurationRepository.GetAllActiveConfigurationsAsync)
  ↓
...
Created comprehensive test suite with **32 new tests** in new `FileProcessing.Infrastructure.Tests` project:
