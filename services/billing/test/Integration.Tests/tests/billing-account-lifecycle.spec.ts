...
test.describe('Billing Account Lifecycle', () => {
  ...
  test('Complete billing account workflow', async ({ request }) => {
    ...
    const getResponse1 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    expect(getResponse1.status()).toBe(404);

    // Step 2: Create billing account
    console.log('📋 Step 2: Creating billing account...');
    const createResponse = await request.post(`${baseUrl}/api/billing/accounts`, {
      data: {
        accountId: accountId,
    ...
    const getResponse2 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const activateResponse = await request.post(`${baseUrl}/api/billing/accounts/${accountId}/activate`);
    expect(activateResponse.status()).toBe(200);
    console.log('✅ Step 4: Account activated');

    // Step 5: Verify account is now Active
    console.log('📋 Step 5: Verifying Active status...');
    const getResponse3 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const payment1Response = await request.post(`${baseUrl}/api/billing/payments`, {
      data: {
        accountId: accountId,
        amount: 150.00,
    ...
    const getResponse4 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const payment2Response = await request.post(`${baseUrl}/api/billing/payments`, {
      data: {
        accountId: accountId,
        amount: 200.00,
    ...
    const getResponse5 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const updatePremiumResponse = await request.put(`${baseUrl}/api/billing/accounts/${accountId}/premium`, {
      data: {
    ...
    const getResponse6 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const suspendResponse = await request.post(`${baseUrl}/api/billing/accounts/${accountId}/suspend`, {
      data: {
    ...
    const getResponse7 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
    ...
    const closeResponse = await request.post(`${baseUrl}/api/billing/accounts/${accountId}/close`, {
      data: {
    ...
    const getResponse8 = await request.get(`${baseUrl}/api/billing/accounts/${accountId}`);
  ...
  test('Get all accounts includes created account', async ({ request }) => {
    // Create an account
    await request.post(`${baseUrl}/api/billing/accounts`, {
      data: {
        accountId: accountId,
        customerId: customerId,
        policyNumber: policyNumber,
    ...
    // Get all accounts
    const getAllResponse = await request.get(`${baseUrl}/api/billing/accounts`);
  ...
  test('Payment validation - negative amount', async ({ request }) => {
    ...
    await request.post(`${baseUrl}/api/billing/accounts/${accountId}/activate`);

    // Try to record negative payment
    const paymentResponse = await request.post(`${baseUrl}/api/billing/payments`, {
      data: {
        accountId: accountId,
        amount: -50.00,
