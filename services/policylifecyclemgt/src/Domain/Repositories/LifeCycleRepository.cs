...
public class LifeCycleRepository : ILifeCycleRepository
{
    ...
    private readonly ILogger<LifeCycleRepository> _logger;

    public LifeCycleRepository(Container container, ILogger<LifeCycleRepository> logger)
    {
        _container = container ?? throw new ArgumentNullException(nameof(container));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    }

    public async Task<Models.LifeCycle?> GetByIdAsync(string policyId)
    {
        try
        {
            var response = await _container.ReadItemAsync<Models.LifeCycle>(
                policyId,
    ...
    public async Task<Models.LifeCycle?> GetByQuoteIdAsync(string quoteId)
    {
        var query = new QueryDefinition(
            "SELECT * FROM c WHERE c.quoteId = @quoteId")
            .WithParameter("@quoteId", quoteId);

        var iterator = _container.GetItemQueryIterator<Models.LifeCycle>(query);
    ...
    public async Task<Models.LifeCycle?> GetByPolicyNumberAsync(string policyNumber)
    {
        var query = new QueryDefinition(
    ...
    public async Task<IEnumerable<Models.LifeCycle>> GetByCustomerIdAsync(string customerId)
    {
        var query = new QueryDefinition(
        ...
        var iterator = _container.GetItemQueryIterator<Models.LifeCycle>(query);
        var policies = new List<Models.LifeCycle>();
    ...
    public async Task<Models.LifeCycle> CreateAsync(Models.LifeCycle policy)
    {
        ...
        _logger.LogInformation(
            "Creating lifecycle document: Id={Id}, PolicyId={PolicyId}, PolicyNumber={PolicyNumber}",
    ...
    public async Task<Models.LifeCycle> UpdateAsync(Models.LifeCycle policy)
    {
        policy.UpdatedUtc = DateTimeOffset.UtcNow;
    ...
    public async Task<IEnumerable<Models.LifeCycle>> GetExpirablePoliciesAsync(DateTimeOffset currentDate)
    {
        var query = new QueryDefinition(
