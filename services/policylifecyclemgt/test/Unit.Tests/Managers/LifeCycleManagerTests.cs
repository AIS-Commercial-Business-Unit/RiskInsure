namespace RiskInsure.PolicyLifeCycleMgt.Test.Unit.Tests.Managers;
...
using RiskInsure.PolicyLifeCycleMgt.Domain.Models;
...
{
    private readonly Mock<ILifeCycleRepository> _repositoryMock;
    private readonly Mock<ILifeCycleNumberGenerator> _numberGeneratorMock;
    private readonly Mock<IMessageSession> _messageSessionMock;
    private readonly Mock<ILogger<LifeCycleManager>> _loggerMock;
    private readonly LifeCycleManager _manager;

    public LifeCycleManagerTests()
    {
        _repositoryMock = new Mock<ILifeCycleRepository>();
        _numberGeneratorMock = new Mock<ILifeCycleNumberGenerator>();
        _messageSessionMock = new Mock<IMessageSession>();
        _loggerMock = new Mock<ILogger<LifeCycleManager>>();

        _manager = new LifeCycleManager(
    ...
    public async Task CreateFromQuoteAsync_NewQuote_CreatesPolicyAndPublishesEvent()
    {
        ...
        _numberGeneratorMock.Setup(g => g.GenerateAsync())
            .ReturnsAsync("LCM-2026-000001");

        _repositoryMock.Setup(r => r.CreateAsync(It.IsAny<LifeCycle>()))
            .ReturnsAsync((LifeCycle p) => p);
        ...
        result.PolicyNumber.Should().Be("LCM-2026-000001");
        result.Status.Should().Be("Bound");
        result.Premium.Should().Be(quoteAccepted.Premium);

        _repositoryMock.Verify(r => r.CreateAsync(It.IsAny<LifeCycle>()), Times.Once);
        _messageSessionMock.Verify(m => m.Publish(
            It.Is<LifeCycleInitiated>(e => e.QuoteId == quoteId),
    ...
    public async Task CreateFromQuoteAsync_DuplicateQuote_ReturnsExistingPolicy()
    {
        ...
        var existingPolicy = new LifeCycle
        {
            Id = policyId,
            PolicyId = policyId,
            QuoteId = quoteId,
        ...
        _repositoryMock.Verify(r => r.CreateAsync(It.IsAny<LifeCycle>()), Times.Never);
        _messageSessionMock.Verify(m => m.Publish(It.IsAny<LifeCycleInitiated>(), It.IsAny<PublishOptions>()), Times.Never);
    }

    [Fact]
    public async Task GetLifeCycleAsync_ExistingPolicy_ReturnsPolicy()
    {
        // Arrange
        var policyId = Guid.NewGuid().ToString();
        ...
        // Act
        var result = await _manager.GetLifeCycleAsync(policyId);
    ...
    [Fact]
    public async Task GetCustomerLifeCyclesAsync_ExistingPolicies_ReturnsAllPolicies()
    {
        // Arrange
        var customerId = "CUST-123";
        ...
        var policies = new List<LifeCycle>
        {
            new() 
            { 
                Id = policyId1,
        ...
        // Act
        var result = await _manager.GetCustomerLifeCyclesAsync(customerId);
