...
- Policy API running on http://localhost:7079
...
- ❌ Retrieving existing policies (requires lifecycle created via QuoteAccepted event)
...
**Business validation errors** (InvalidLifeCycleStatus, LifeCycleNotFound) use custom format:
```typescript
expect(error.error).toBe('InvalidLifeCycleStatus');
...
- Base URL (default: http://localhost:7079)
...
- Ensure API is running: `cd services/policylifecyclemgt/src/Api && dotnet run`
- Check port 7079 is available
