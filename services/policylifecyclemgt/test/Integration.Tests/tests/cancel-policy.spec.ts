...
test.describe('Cancel Policy', () => {
  ...
  test('should return 404 when policy not found', async ({ request }) => {
    ...
    const response = await request.post(`/api/lifecycles/${nonExistentPolicyId}/cancel`, {
      data: {
        reason: 'Test',
    ...
    expect(error.error).toBe('LifeCycleNotFound');
  ...
  test('should validate required fields', async ({ request }) => {
    ...
    const response = await request.post(`/api/lifecycles/${policyId}/cancel`, {
      data: {
        // reason: "Unhappy",
        cancellationDate: new Date().toISOString()
      }
