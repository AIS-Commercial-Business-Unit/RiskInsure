...
|-------------|----------|-------------|
| POST | `/api/lifecycles/{policyId}/issue` | Issue policy after payment method added |
| GET | `/api/lifecycles/{policyId}` | Retrieve policy details |
| GET | `/api/customers/{customerId}/lifecycles` | Retrieve all policies for a customer |
| POST | `/api/lifecycles/{policyId}/cancel` | Cancel policy before expiration |
| POST | `/api/lifecycles/{policyId}/reinstate` | Reinstate lapsed policy |
...
public class LifeCycle
{
    ...
    public string PolicyNumber { get; set; }  // LCM-2026-000001
...
### QuoteAcceptedLifeCycleHandler

**Purpose**: Handle `QuoteAccepted` event from Rating & Underwriting domain

**Handler Location**: `services/policy/src/Endpoint.In/Handlers/QuoteAcceptedLifeCycleHandler.cs`

**Process**:
1. Receive `QuoteAccepted` event via RabbitMQ transport
2. Check if policy already exists for quoteId (idempotency)
3. Generate unique policyId and lifecycle number
4. Create lifecycle document with coverage terms from event
5. Set status to "Bound"
6. Save policy to Cosmos DB
7. Publish `LifeCycleInitiated` event
...
public class QuoteAcceptedLifeCycleHandler : IHandleMessages<QuoteAccepted>
{
    ...
    private readonly ILifeCycleNumberGenerator _policyNumberGenerator;
    ...
    public async Task Handle(QuoteAccepted message, IMessageHandlerContext context)
    {
        ...
        await context.Publish(new LifeCycleInitiated(
...
**Events Published**: `LifeCycleInitiated`
...
7. Generate lifecycle documents (PDF)

**Response**: `200 OK`
```json
{
  "policyId": "guid",
  "policyNumber": "LCM-2026-000001",
  "status": "Issued",
...
4. Save policy
5. Publish `LifeCycleCancelled` event (triggers refund in Billing)
...
**Events Published**: `LifeCycleCancelled`
...
**Events Published**: `LifeCycleReinstated`
...
public interface ILifeCycleManager
{
    ...
    Task<Policy> IssueLifeCycleAsync(string policyId);
    Task<Policy> CancelLifeCycleAsync(string policyId, DateTimeOffset cancellationDate, string reason);
    Task<Policy> ReinstateLifeCycleAsync(string policyId);
    Task<Policy> GetLifeCycleAsync(string policyId);
...
### LifeCycleNumberGenerator

**Interface**: `ILifeCycleNumberGenerator`
...
**Contract Location**: `services/policy/src/Domain/Contracts/Events/LifeCycleInitiated.cs`
