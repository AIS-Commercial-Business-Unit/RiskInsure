...
2. Create lifecycle document with coverage terms from quote
3. Assign unique `PolicyId` and lifecycle number
4. Set policy status to "Bound" (awaiting payment)
5. Publish `LifeCycleInitiated` event

**Business Rules**:
- Each policy receives unique `PolicyId` (GUID)
- Policy number follows format: `LCM-{YEAR}-{SEQUENCE}` (e.g., LCM-2026-000001)
...
4. Generate lifecycle documents (PDF)
5. Publish `PolicyIssued` event
6. Send lifecycle documents to customer
...
- **Bound**: Quote accepted, lifecycle created, awaiting payment method
...
5. Publish `LifeCycleCancelled` event
...
5. New lifecycle created for next term
...
6. Publish `LifeCycleReinstated` event
...
**When**: Quote accepted and lifecycle document created
...
- **LifeCycleCancelled** → Billing domain (calculate refund)
...
- **Policy Number**: Human-readable policy identifier (LCM-2026-000001)
