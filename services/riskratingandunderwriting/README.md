...
|---------|------|---------|
| API | 7081 | HTTP endpoints for quote management |
...
**Container**: `riskratingandunderwriting`  
...
public class Quote
{
    ...
    public DateTimeOffset? UnderwritingRiskSubmittedUtc { get; set; }
...
### POST /api/risk-quotes/{quoteId}/submit-underwriting
...
### POST /api/risk-quotes/{quoteId}/accept
...
### GET /api/customers/{customerId}/risk-quotes
...
cd services/riskratingandunderwriting/src/Api
dotnet run
```

API will be available at: `http://localhost:7081`  
Scalar API documentation: `http://localhost:7081/scalar/v1`

### Starting Endpoint.In (Optional)

```powershell
# Terminal 2: Start message endpoint
cd services/riskratingandunderwriting/src/Endpoint.In
dotnet run
```
...
**Unit Tests**:
```powershell
cd services/riskratingandunderwriting/test/Unit.Tests
dotnet test
```
...
```powershell
# First-time setup
cd services/riskratingandunderwriting/test/Integration.Tests
npm install
npx playwright install chromium

# Run tests (API must be running on localhost:7081)
npm test                  # Headless
...
**Rating & Underwriting CAN create its own test data** via POST /api/risk-quotes/start, so integration tests can test full happy paths:
...
test('should create, underwrite, and accept quote', async ({ request }) => {
  // Create quote
  const createResponse = await request.post('/api/risk-quotes/start', {
    data: {
      customerId: 'CUST-001',
      structureCoverageLimit: 300000,
      structureDeductible: 1000,
  ...
  // Submit underwriting
  const underwritingResponse = await request.post(`/api/risk-quotes/${quoteId}/submit-underwriting`, {
    data: {
      priorClaimsCount: 0,
      propertyAgeYears: 10,
      creditTier: 'Excellent'
  ...
  // Accept quote
  const acceptResponse = await request.post(`/api/risk-quotes/${quoteId}/accept`);
...
```powershell
dotnet build services/riskratingandunderwriting/src/Domain/Domain.csproj
dotnet build services/riskratingandunderwriting/src/Infrastructure/Infrastructure.csproj
dotnet build services/riskratingandunderwriting/src/Api/Api.csproj
dotnet build services/riskratingandunderwriting/src/Endpoint.In/Endpoint.In.csproj
dotnet build services/riskratingandunderwriting/test/Unit.Tests/Unit.Tests.csproj
```

### Test Coverage

**Unit Tests**: 12 tests
- RiskUnderwritingEngine: 5 tests (Class A, Class B, decline scenarios)
- RiskRatingEngine: 7 tests (premium calculation, factor calculations)
...
✅ **Principle VII - Thin Message Handlers**: Future handlers delegate to RiskQuoteManager  
