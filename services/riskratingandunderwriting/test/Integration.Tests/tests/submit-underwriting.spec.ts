...
test.describe('Submit Underwriting', () => {
  ...
  test('should approve Class A underwriting', async ({ request }) => {
    const response = await request.post(`/api/risk-quotes/${quoteId}/submit-underwriting`, {
      data: {
        priorClaimsCount: 0,
        propertyAgeYears: 10,
  ...
  test('should return 404 for non-existent quote', async ({ request }) => {
    ...
    const response = await request.post(`/api/risk-quotes/${nonExistentId}/submit-underwriting`, {
      data: {
        priorClaimsCount: 0,
        propertyAgeYears: 10,
        creditTier: 'Excellent'
