...
test.describe('Get Customer Quotes', () => {
    ...
    test('should return empty list for customer with no quotes', async ({ request }) => {
        const response = await request.get(`/api/customers/${customerId}/risk-quotes`);
    ...
    test('should return all quotes for customer', async ({ request }) => {
        ...
        for (let i = 0; i < 3; i++) {
            const response = await request.post('/api/risk-quotes/start', {
                data: {
                    customerId,
    ...
    test('should return quote summaries with correct fields', async ({ request }) => {
        // Create a quote
        const startResponse = await request.post('/api/risk-quotes/start', {
            data: {
                customerId,
                structureCoverageLimit: 200000,
                structureDeductible: 1000,
    ...
    test('should include premium in quoted quotes', async ({ request }) => {
        ...
        // Submit underwriting
        await request.post(`/api/risk-quotes/${quoteId}/submit-underwriting`, {
            data: {
                priorClaimsCount: 0,
                propertyAgeYears: 10,
    ...
    test('should return quotes in different statuses', async ({ request }) => {
        ...
        // Create draft quote
        const draftResponse = await request.post('/api/risk-quotes/start', {
            data: {
                customerId,
                structureCoverageLimit: 200000,
                structureDeductible: 1000,
        ...
        const quotedResponse = await request.post('/api/risk-quotes/start', {
            data: {
                customerId,
        ...
        await request.post(`/api/risk-quotes/${quotedResult.quoteId}/submit-underwriting`, {
            data: {
                priorClaimsCount: 0,
                propertyAgeYears: 10,
                creditTier: 'Excellent'
        ...
        const acceptedResponse = await request.post('/api/risk-quotes/start', {
            data: {
                customerId,
        ...
        await request.post(`/api/risk-quotes/${acceptedResult.quoteId}/submit-underwriting`, {
            data: {
                priorClaimsCount: 0,
                propertyAgeYears: 10,
                creditTier: 'Excellent'
        ...
        await request.post(`/api/risk-quotes/${acceptedResult.quoteId}/accept`);
    ...
    test('should not return quotes from other customers', async ({ request }) => {
        ...
        const response1 = await request.post('/api/risk-quotes/start', {
            data: {
                customerId,
                structureCoverageLimit: 200000,
                structureDeductible: 1000,
        ...
        await request.post('/api/risk-quotes/start', {
            data: {
