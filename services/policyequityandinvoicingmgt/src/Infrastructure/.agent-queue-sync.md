...
## Purpose
Keep `queues.ps1` synchronized with message handlers and event publishers in the PolicyEquityAndInvoicing service.
...
- Extract endpoint names (e.g., "RiskInsure.PolicyEquityAndInvoicingMgt.Api", "RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint")
```
...
- Extract full type name of T (e.g., RiskInsure.PolicyEquityAndInvoicingMgt.Domain.Contracts.Commands.RecordPayment)
...
**Agent Response:**
1. Scans `services/policyequityandinvoicingmgt/src/Endpoint.In/Handlers/` for handlers
...
**Service:** PolicyEquityAndInvoicing  
**Endpoints Found:**
- RiskInsure.PolicyEquityAndInvoicingMgt.Api (send-only)
- RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint

**Handlers Found:**
- RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint handles:
  - RiskInsure.PolicyEquityAndInvoicingMgt.Domain.Contracts.Commands.RecordPayment

**Events Published:**
- RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint publishes:
  - RiskInsure.PublicContracts.Contracts.Events.PaymentReceived

**Generated queues.ps1:**
```powershell
# RabbitMQ Topology Setup for PolicyEquityAndInvoicing Service
$env:RABBITMQ_CONNECTION_STRING = 'host=localhost;username=guest;password=guest'
...
# PolicyEquityAndInvoicingMgt Endpoints
rabbitmqadmin declare queue name=RiskInsure.PolicyEquityAndInvoicingMgt.Api durable=true
rabbitmqadmin declare queue name=RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint durable=true

# Command routing documentation
# RiskInsure.PolicyEquityAndInvoicingMgt.Domain.Contracts.Commands.RecordPayment -> RiskInsure.PolicyEquityAndInvoicingMgt.Endpoint
