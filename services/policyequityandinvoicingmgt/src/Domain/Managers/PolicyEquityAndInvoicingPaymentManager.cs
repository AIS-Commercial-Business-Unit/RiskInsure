...
/// </summary>
public class PolicyEquityAndInvoicingPaymentManager : IPolicyEquityAndInvoicingPaymentManager
{
    private readonly IPolicyEquityAndInvoicingAccountRepository _repository;
    private readonly IMessageSession _messageSession;
    private readonly ILogger<PolicyEquityAndInvoicingPaymentManager> _logger;

    // Business rule constants
    private const decimal MinimumPaymentAmount = 1.00m;

    public PolicyEquityAndInvoicingPaymentManager(
        IPolicyEquityAndInvoicingAccountRepository repository,
        IMessageSession messageSession,
        ILogger<PolicyEquityAndInvoicingPaymentManager> logger)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                _logger.LogError(
                    "PolicyEquityAndInvoicingAccount {AccountId} not found for payment recording",
                    dto.AccountId);
                return PaymentRecordingResult.Failure(
            ...
            if (account.Status != PolicyEquityAndInvoicingAccountStatus.Active)
            {
                _logger.LogWarning(
            ...
            var attempt = 0;
            PolicyEquityAndInvoicingAccount? updatedAccount = null;
