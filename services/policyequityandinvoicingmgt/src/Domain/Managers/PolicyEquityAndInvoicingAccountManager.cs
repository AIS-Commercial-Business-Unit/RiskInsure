...
using NServiceBus;
using RiskInsure.PolicyEquityAndInvoicingMgt.Domain.Contracts.Events;
...
/// </summary>
public class PolicyEquityAndInvoicingAccountManager : IPolicyEquityAndInvoicingAccountManager
{
    private readonly IPolicyEquityAndInvoicingAccountRepository _repository;
    private readonly IMessageSession _messageSession;
    private readonly ILogger<PolicyEquityAndInvoicingAccountManager> _logger;

    public PolicyEquityAndInvoicingAccountManager(
        IPolicyEquityAndInvoicingAccountRepository repository,
        IMessageSession messageSession,
        ILogger<PolicyEquityAndInvoicingAccountManager> logger)
    {
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> CreatePolicyEquityAndInvoicingAccountAsync(
        ...
        try
        {
            ...
            if (existingAccount != null)
            {
                _logger.LogWarning(
                    ...
                    dto.AccountId);
                return PolicyEquityAndInvoicingAccountResult.Success(dto.AccountId);
            }
            ...
            {
                return PolicyEquityAndInvoicingAccountResult.Failure(
            ...
            var account = new PolicyEquityAndInvoicingAccount
            {
                ...
                TotalPaid = 0,
                Status = PolicyEquityAndInvoicingAccountStatus.Pending, // Created in Pending state
            ...
            // Publish PolicyEquityAndInvoicingAccountCreated event
            var @event = new PolicyEquityAndInvoicingAccountCreated(
                ...
                PolicyEquityAndInvoicingCycle: account.PolicyEquityAndInvoicingCycle,
            ...
            return PolicyEquityAndInvoicingAccountResult.Success(account.AccountId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> UpdatePremiumOwedAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (account.Status == PolicyEquityAndInvoicingAccountStatus.Closed)
            {
                return PolicyEquityAndInvoicingAccountResult.Failure(
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> ActivateAccountAsync(
        string accountId,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (account.Status != PolicyEquityAndInvoicingAccountStatus.Pending)
            {
                _logger.LogWarning(
                    "Account {AccountId} is already in {Status} status",
                    accountId, account.Status);
                return PolicyEquityAndInvoicingAccountResult.Success(accountId); // Idempotent
            }

            // Activate account
            account.Status = PolicyEquityAndInvoicingAccountStatus.Active;
            account.LastUpdatedUtc = DateTimeOffset.UtcNow;
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> SuspendAccountAsync(
        string accountId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (account.Status == PolicyEquityAndInvoicingAccountStatus.Suspended)
            {
                _logger.LogWarning(
                    "Account {AccountId} is already suspended",
                    accountId);
                return PolicyEquityAndInvoicingAccountResult.Success(accountId);
            }

            // Suspend account
            account.Status = PolicyEquityAndInvoicingAccountStatus.Suspended;
            account.LastUpdatedUtc = DateTimeOffset.UtcNow;
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> CloseAccountAsync(
        string accountId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Close account
            account.Status = PolicyEquityAndInvoicingAccountStatus.Closed;
            account.LastUpdatedUtc = DateTimeOffset.UtcNow;
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccountResult> UpdatePolicyEquityAndInvoicingCycleAsync(
        UpdatePolicyEquityAndInvoicingCycleDto dto,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation(
            "Updating billing cycle for account {AccountId} to {NewCycle}",
            dto.AccountId, dto.NewPolicyEquityAndInvoicingCycle);
        ...
        try
        {
            ...
            var oldCycle = account.PolicyEquityAndInvoicingCycle;

            // Update billing cycle
            account.PolicyEquityAndInvoicingCycle = dto.NewPolicyEquityAndInvoicingCycle;
            account.LastUpdatedUtc = DateTimeOffset.UtcNow;

            await _repository.UpdateAsync(account, cancellationToken);

            // Publish PolicyEquityAndInvoicingCycleUpdated event
            var @event = new PolicyEquityAndInvoicingCycleUpdated(
                MessageId: Guid.NewGuid(),
                OccurredUtc: DateTimeOffset.UtcNow,
                AccountId: account.AccountId,
                OldPolicyEquityAndInvoicingCycle: oldCycle,
                NewPolicyEquityAndInvoicingCycle: dto.NewPolicyEquityAndInvoicingCycle,
                ChangeReason: dto.ChangeReason,
            ...
            _logger.LogInformation(
                ...
                account.AccountId, oldCycle, dto.NewPolicyEquityAndInvoicingCycle);
    ...
    /// </summary>
    public async Task<IEnumerable<PolicyEquityAndInvoicingAccount>> GetAllAccountsAsync(
        CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    public async Task<PolicyEquityAndInvoicingAccount?> GetAccountByIdAsync(
        string accountId, 
        CancellationToken cancellationToken = default)
    {
