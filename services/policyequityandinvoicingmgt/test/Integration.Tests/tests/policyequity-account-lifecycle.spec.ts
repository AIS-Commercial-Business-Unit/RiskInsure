...
/**
 * policyequity Account Lifecycle Integration Tests
 * 
 * Tests the complete workflow of a policyequity account from creation to closure.
 * Requires the policyequity API to be running on http://localhost:7071
 * 
 * Run with:
 *   npm test
...
test.describe('policyequity Account Lifecycle', () => {
  ...
  test('Complete policyequity account workflow', async ({ request }) => {
    ...
    const getResponse1 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    expect(getResponse1.status()).toBe(404);

    // Step 2: Create policyequity account
    console.log('📋 Step 2: Creating policyequity account...');
    const createResponse = await request.post(`/api/policyequityandinvoicingmgt/accounts`, {
      data: {
        accountId: accountId,
    ...
    const getResponse2 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const activateResponse = await request.post(`/api/policyequityandinvoicingmgt/accounts/${accountId}/activate`);
    expect(activateResponse.status()).toBe(200);
    console.log('✅ Step 4: Account activated');

    // Step 5: Verify account is now Active
    console.log('📋 Step 5: Verifying Active status...');
    const getResponse3 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const payment1Response = await request.post(`/api/policyequityandinvoicingmgt/payments`, {
      data: {
        accountId: accountId,
        amount: 150.00,
    ...
    const getResponse4 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const payment2Response = await request.post(`/api/policyequityandinvoicingmgt/payments`, {
      data: {
        accountId: accountId,
        amount: 200.00,
    ...
    const getResponse5 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const updatePremiumResponse = await request.put(`/api/policyequityandinvoicingmgt/accounts/${accountId}/premium`, {
      data: {
    ...
    const getResponse6 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const suspendResponse = await request.post(`/api/policyequityandinvoicingmgt/accounts/${accountId}/suspend`, {
      data: {
    ...
    const getResponse7 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
    ...
    const closeResponse = await request.post(`/api/policyequityandinvoicingmgt/accounts/${accountId}/close`, {
      data: {
    ...
    const getResponse8 = await request.get(`/api/policyequityandinvoicingmgt/accounts/${accountId}`);
  ...
  test('Get all accounts includes created account', async ({ request }) => {
    // Create an account
    await request.post(`/api/policyequityandinvoicingmgt/accounts`, {
      data: {
        accountId: accountId,
        customerId: customerId,
        policyNumber: policyNumber,
    ...
    // Get all accounts
    const getAllResponse = await request.get(`/api/policyequityandinvoicingmgt/accounts`);
  ...
  test('Payment validation - negative amount', async ({ request }) => {
    ...
    await request.post(`/api/policyequityandinvoicingmgt/accounts/${accountId}/activate`);

    // Try to record negative payment
    const paymentResponse = await request.post(`/api/policyequityandinvoicingmgt/payments`, {
      data: {
        accountId: accountId,
        amount: -50.00,
