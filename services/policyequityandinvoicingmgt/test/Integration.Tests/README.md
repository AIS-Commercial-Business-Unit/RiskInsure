...
This directory contains Playwright-based integration tests for the PolicyEquityAndInvoicingMgt API.

## Prerequisites

- Node.js 18+ installed
- PolicyEquityAndInvoicingMgt API running on `http://localhost:7071`
...
**Start the PolicyEquityAndInvoicingMgt API:**
```bash
...
npx playwright test tests/policyequity-account-lifecycle.spec.ts

# Run tests matching a pattern
npx playwright test --grep "payment"

# Run a single test by name
npx playwright test --grep "Complete policy equity account workflow"
```
...
- Check API is accessible: Open `http://localhost:7071/api/policyequityandinvoicingmgt/payments/health` in browser
...
See `tests/policyequity-account-lifecycle.spec.ts` for examples. Each test:
...
```bash
npx playwright test --debug -g "Complete policy equity account workflow"
```
