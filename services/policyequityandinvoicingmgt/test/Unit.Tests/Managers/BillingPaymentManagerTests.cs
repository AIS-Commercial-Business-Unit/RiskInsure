...
/// <summary>
/// Unit tests for PolicyEquityAndInvoicingPaymentManager business logic.
/// Tests business rules, validation, and orchestration without external dependencies.
/// </summary>
public class PolicyEquityAndInvoicingPaymentManagerTests
{
    ...
    private readonly Mock<ILogger<PolicyEquityAndInvoicingPaymentManager>> _mockLogger;
    private readonly PolicyEquityAndInvoicingPaymentManager _manager;

    public PolicyEquityAndInvoicingPaymentManagerTests()
    {
        _mockRepository = new Mock<IPolicyEquityAndInvoicingAccountRepository>();
        _mockMessageSession = new Mock<IMessageSession>();
        _mockLogger = new Mock<ILogger<PolicyEquityAndInvoicingPaymentManager>>();
        
        _manager = new PolicyEquityAndInvoicingPaymentManager(
    ...
    public async Task RecordPaymentAsync_ValidPayment_ReturnsSuccess()
    {
        ...
        _mockRepository.Verify(r => r.UpdateAsync(
            It.Is<PolicyEquityAndInvoicingAccount>(a => a.TotalPaid == 150m), 
    ...
    public async Task RecordPaymentAsync_InactiveAccount_ReturnsFailure()
    {
        ...
        var account = new PolicyEquityAndInvoicingAccount
        {
            ...
            CurrentPremiumOwed = 500m,
            TotalPaid = 0m,
            Status = PolicyEquityAndInvoicingAccountStatus.Suspended, // Not Active
            PolicyEquityAndInvoicingCycle = PolicyEquityAndInvoicingCycle.Monthly,
            EffectiveDate = DateTimeOffset.UtcNow,
            CreatedUtc = DateTimeOffset.UtcNow,
            LastUpdatedUtc = DateTimeOffset.UtcNow
