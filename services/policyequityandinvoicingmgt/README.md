# PolicyEquityAndInvoicingMgt Service

Policy equity, invoicing, and payment tracking service.
...
- **Models**: `PolicyEquityAndInvoicingAccount` - Core billing entity
- **services/policyequityandinvoicingmgtDb**: 
  - `IPolicyEquityAndInvoicingAccountRepository` - Repository interface
  - `PolicyEquityAndInvoicingAccountRepository` - Cosmos DB implementation with optimistic concurrency
  - `PolicyEquityAndInvoicingAccountDocument` - Cosmos DB document model
...
### API Layer (`src/Api/`)
- **Controllers**: `BillingController` - POST /api/policyequityandinvoicingmgt/payments (202 Accepted pattern)
...
**Cosmos DB Container**: `PolicyEquityAndInvoicingMgt`  
**Partition Key**: `/accountId`  
**Document Type**: `PolicyEquityAndInvoicingAccount`

### PolicyEquityAndInvoicingAccount Properties
...
- `Status` (PolicyEquityAndInvoicingAccountStatus enum) - Active, Suspended, PaidInFull, Cancelled
...
- Interface defined in Domain (`IPolicyEquityAndInvoicingAccountRepository`)
- Implementation in Domain (`PolicyEquityAndInvoicingAccountRepository`)
...
3. Create Cosmos DB container:
   ```powershell
   # Database: RiskInsure
   # Container: PolicyEquityAndInvoicingMgt
   # Partition key: /accountId
   ```
...
```powershell
# POST payment
curl -X POST http://localhost:5000/api/policyequityandinvoicingmgt/payments `
...
### Unit Tests (TODO)
- `Domain.Tests`: Test PolicyEquityAndInvoicingAccount business logic
