# Multi-Policy PolicyEquityAndInvoicing - Business Requirements

**Domain**: PolicyEquityAndInvoicing  
**Version**: 2.0.0  
**Date**: February 5, 2026  
**Updates**: Added multi-policy PolicyEquityAndInvoicing requirements

---

## Business Purpose

The PolicyEquityAndInvoicing domain manages **PolicyEquityAndInvoicing accounts, payment tracking, and financial transactions** for insurance policies. It supports **multi-policy PolicyEquityAndInvoicing** where customers with multiple active policies have a consolidated PolicyEquityAndInvoicing account with per-policy premium tracking and targeted payment allocation.

---

## Business Capabilities

### 1. PolicyEquityAndInvoicing Account Management

**Capability**: Create and maintain PolicyEquityAndInvoicing accounts associated with customer policies.

**Account Creation**:
- Triggered by `PolicyIssued` event from Policy domain
- One PolicyEquityAndInvoicing account per customer (supports multiple policies)
- Each policy added as a "PolicyEquityAndInvoicing line item" with its own premium and balance

**Business Rules**:
- PolicyEquityAndInvoicingAccountId is unique (GUID)
...
### 2. Multi-Policy PolicyEquityAndInvoicing (MVP Requirement)

**Capability**: Manage multiple policies under a single PolicyEquityAndInvoicing account with per-policy premium tracking.
...
**Business Rules**:
- Customer can have multiple active policies in one PolicyEquityAndInvoicing account
...
- Both policies added to same PolicyEquityAndInvoicing account
...
5. Update PolicyEquityAndInvoicing status to "PaymentPending"
...
- PolicyEquityAndInvoicing account established for customer
...
**Business Significance**:
- Multi-policy PolicyEquityAndInvoicing activated
...
- **PolicyIssued** (from Policy domain): Create PolicyEquityAndInvoicing account or add policy
...
- **PolicyEquityAndInvoicingAccountCreated**: PolicyEquityAndInvoicing account established
- **PaymentRecorded**: Payment processed and balance updated
- **PolicyAdded**: Additional policy added to PolicyEquityAndInvoicing account
...
**Flow**:
1. Customer has Policy A ($1,200 annual) with PolicyEquityAndInvoicing account
2. Customer purchases Policy B ($800 annual)
3. `PolicyIssued` event received for Policy B
4. Policy B added to existing PolicyEquityAndInvoicing account
5. TotalBalance updated: $1,200 → $2,000
6. `PolicyAdded` event published

**Result**: One PolicyEquityAndInvoicing account with two policies, aggregate balance $2,000
...
5. **PolicyEquityAndInvoicing History**:
...
- **PolicyEquityAndInvoicingAccountId**: Unique identifier for a customer's PolicyEquityAndInvoicing account
