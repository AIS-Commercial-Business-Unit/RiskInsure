# PolicyEquityAndInvoicing Payment Manager

**Domain**: PolicyEquityAndInvoicing  
**Aggregate Root**: PolicyEquityAndInvoicingAccount  
**Version**: 1.0.0  
...
**Responsibilities**: 
- Recording customer payments against PolicyEquityAndInvoicing accounts
...
Records a customer payment against their PolicyEquityAndInvoicing account, reducing the outstanding balance and tracking the payment reference for audit purposes. This capability ensures that only valid payments are processed while maintaining accurate account balances and providing complete payment traceability.
...
| 3 | PolicyEquityAndInvoicing account must exist in the system | ACCOUNT_NOT_FOUND | No |
...
|-------|------------------|----------|-------------|
| Account ID | Unique identifier for the PolicyEquityAndInvoicing account | Yes | Must exist in system |
...
**Success**: 
- Payment successfully recorded in PolicyEquityAndInvoicing account
...
**Failure**: 
- No changes made to PolicyEquityAndInvoicing account
...
**Purpose**: Notifies downstream systems that a payment has been successfully recorded against a PolicyEquityAndInvoicing account, enabling coordinated processing across the platform.
...
| AccountId | PolicyEquityAndInvoicing account that received the payment | ACC-12345 |
...
|------------|------------------|------|
| PolicyEquityAndInvoicingAccountRepository | Manages PolicyEquityAndInvoicing account data, payment records, and account balance updates with optimistic concurrency control | Repository |
...
**Given**: 
- Active PolicyEquityAndInvoicing account ACC-12345 with outstanding balance of $1,000.00
...
**When**: Payment is processed through the PolicyEquityAndInvoicing system
...
**Given**: 
- Active PolicyEquityAndInvoicing account ACC-67890
...
**Given**: 
- Active PolicyEquityAndInvoicing account ACC-11111
...
**Given**: 
- PolicyEquityAndInvoicing account ACC-55555 with status: Suspended
