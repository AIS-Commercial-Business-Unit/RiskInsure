...
namespace FileProcessing.Integration.Tests.Api;

public class ConfigurationScheduledFtpE2ETests
{
    private const string FtpContainerName = "file-processing-ftp";
    ...
    {
        var apiBaseUrl = Environment.GetEnvironmentVariable("FILE_PROCESSING_API_BASE_URL") ?? DefaultApiBaseUrl;
        var jwtSecret = Environment.GetEnvironmentVariable("FILE_PROCESSING_JWT_SECRET")
            ?? "REPLACE_WITH_SECRET_KEY_AT_LEAST_32_CHARS_LONG_FOR_PRODUCTION";
        var jwtIssuer = Environment.GetEnvironmentVariable("FILE_PROCESSING_JWT_ISSUER")
            ?? "https://riskinsure.com";
        var jwtAudience = Environment.GetEnvironmentVariable("FILE_PROCESSING_JWT_AUDIENCE")
            ?? "file-processing-api";
        var ftpPasswordSecretName = Environment.GetEnvironmentVariable("FILE_PROCESSING_FTP_PASSWORD_SECRET_NAME")
            ?? "test-ftp-password";
