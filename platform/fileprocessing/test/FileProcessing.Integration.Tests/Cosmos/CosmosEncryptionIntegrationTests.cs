...
namespace FileProcessing.Integration.Tests.Cosmos;
...
public class CosmosEncryptionIntegrationTests : IAsyncLifetime
{
    ...
    public async Task DisposeAsync()
    {
        ...
        try
        {
            if (_cosmosContext != null)
            {
                ...
                var iterator = container.GetItemQueryIterator<FileProcessingConfiguration>(query);
                while (iterator.HasMoreResults)
                {
                    var results = await iterator.ReadNextAsync();
                    foreach (var item in results)
                    {
                        await container.DeleteItemAsync<FileProcessingConfiguration>(item.Id.ToString(), new PartitionKey(ClientId));
                    }
                }
            }
        }
    ...
    public async Task CosmosEncryptionConfiguration_InitializesSuccessfully()
    {
        ...
        var metadata = await encryptionConfig.GetEncryptionPolicyMetadataForFileProcessingConfigs();
    ...
    public async Task FtpProtocolSettings_WithEncryption_StoresCredentialsSecurely()
    {
        ...
        var config = new FileProcessingConfiguration
        {
            Id = Guid.NewGuid(),
            ClientId = ClientId,
    ...
    public async Task MultipleConfigurations_CanBeStoredWithEncryption()
    {
        ...
        var configs = new List<FileProcessingConfiguration>
        {
        ...
        // Act
        var results = new List<ItemResponse<FileProcessingConfiguration>>();
    ...
    private static FileProcessingConfiguration CreateTestConfiguration(
