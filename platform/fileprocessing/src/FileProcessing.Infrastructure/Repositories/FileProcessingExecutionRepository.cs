...
/// <summary>
/// T037: Cosmos DB implementation of IFileProcessingExecutionRepository
/// </summary>
public class FileProcessingExecutionRepository : IFileProcessingExecutionRepository
{
    private readonly CosmosDbContext _context;
    private readonly ILogger<FileProcessingExecutionRepository> _logger;

    public FileProcessingExecutionRepository(
        CosmosDbContext context,
        ILogger<FileProcessingExecutionRepository> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<FileProcessingExecution> CreateAsync(
        FileProcessingExecution execution,
    ...
    public async Task<FileProcessingExecution?> GetByIdAsync(
        string clientId,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var response = await _context.ExecutionsContainer.ReadItemAsync<FileProcessingExecution>(
    ...
    public async Task<IReadOnlyList<FileProcessingExecution>> GetByConfigurationAsync(
        string clientId,
        Guid configurationId,
        int pageSize = 50,
        CancellationToken cancellationToken = default)
    {
        ...
        var iterator = _context.ExecutionsContainer.GetItemQueryIterator<FileProcessingExecution>(
            query,
            requestOptions: new QueryRequestOptions { PartitionKey = new PartitionKey(clientId) });

        var results = new List<FileProcessingExecution>();
    ...
    public async Task<IReadOnlyList<FileProcessingExecution>> GetByConfigurationAndDateRangeAsync(
        string clientId,
        Guid configurationId,
    ...
    public async Task<(IReadOnlyList<FileProcessingExecution> Executions, string? ContinuationToken)> GetExecutionHistoryAsync(
        string clientId,
        Guid configurationId,
        int pageSize = 50,
    ...
    public async Task<FileProcessingExecution> UpdateAsync(
        FileProcessingExecution execution,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation(
