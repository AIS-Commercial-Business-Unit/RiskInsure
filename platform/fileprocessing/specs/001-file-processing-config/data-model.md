...
**FileProcessingConfiguration** - No changes
...
**Location**: `src/FileProcessing.Contracts/Commands/RetrieveFile.cs`
...
public record RetrieveFile : ICommand
{
    ...
    // File Processing specific (existing)
...
| `CorrelationId` | string | Request correlation ID | From `RetrieveFile` command |
...
**Container**: `file-processing` (existing)  
**Partition Key**: `/clientId` (existing)  
**Document Types**: `FileProcessingConfiguration`, `FileProcessingExecution`, `DiscoveredFile` (all existing)
...
**Repository**: `IFileProcessingConfigurationRepository` (existing method)  
...
- `Send(RetrieveFile)` - Send command to Worker endpoint
- Throws exception if messaging infrastructure unavailable

**IFileProcessingConfigurationRepository** (Domain):
...
**RetrieveFile** - Adding nullable `TriggeredBy` field:
