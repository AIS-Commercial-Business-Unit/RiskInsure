...
**Question**: How should the API endpoint extract and validate user identity for audit trail in the `RetrieveFileTriggered` event?
...
**Question**: Should `RetrieveFileTriggered` event be published BEFORE or AFTER calling `RetrieveFileService.ExecuteCheckAsync()`?

**Findings**:
- Spec requirement (AC-4): "When the handler begins processing, then the system publishes a RetrieveFileTriggered event **before executing the file check**"
- Existing handler structure (RetrieveFileHandler.cs):
  - Line 40-46: Load configuration
  - Line 48-87: Check IsActive
  - Line 89-93: Call `RetrieveFileService.ExecuteCheckAsync()` (the actual file check)
  - Line 95-129: Process discovered files and publish `RetrieveFileCompleted`
- Event should capture "intention to check" not "result of check" (covered by RetrieveFileCompleted/RetrieveFileFailed)

**Decision**: Publish `RetrieveFileTriggered` event immediately after loading configuration and validating IsActive, before calling `RetrieveFileService.ExecuteCheckAsync()`.
...
### R3: Idempotency Strategy for RetrieveFileTriggered Event

**Question**: How to ensure exactly-once semantics for `RetrieveFileTriggered` event when handler is replayed?
...
- Existing event patterns use `IdempotencyKey` field:
  - `RetrieveFileCompleted`: `"{ClientId}:{ConfigurationId}:completed:{ExecutionId}"` (line 116)
  - `RetrieveFileFailed`: `"{ClientId}:{ConfigurationId}:failed:{ExecutionId}"` (line 61)
- ExecutionId is generated by `RetrieveFileService.ExecuteCheckAsync()` (inside the service call)
...
**Decision**: Create `TriggerRetrieveFileResponse` DTO with minimal fields:
```csharp
...
**Findings**:
- Current implementation: `RetrieveFileService.ExecuteCheckAsync()` generates ExecutionId internally (inside service)
...
2. Pass ExecutionId to `RetrieveFileService.ExecuteCheckAsync()` as new parameter
3. Service uses provided ExecutionId instead of generating new one
4. Use ExecutionId in `RetrieveFileTriggered` event IdempotencyKey

**Implementation Changes Required**:
- Modify `RetrieveFileService.ExecuteCheckAsync()` signature to accept `Guid executionId`
...
- `contracts/RetrieveFileTriggered.md` - Event contract specification
