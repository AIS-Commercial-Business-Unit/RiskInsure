# Monitoring Dashboard: File Processing Service

**T131**: This document provides queries and guidance for monitoring the File Processing Service using Application Insights and Azure Monitor.

## Overview

The File Processing Service exposes metrics and logs through:
...
```kql
customMetrics
| where name == "file_processing_checks_executed_total"
...
```kql
customMetrics
| where name == "file_processing_check_duration_seconds"
...
```kql
customMetrics
| where name == "file_processing_files_discovered_total"
| extend protocol = tostring(customDimensions.protocol)
...
```kql
customMetrics
| where name == "file_processing_active_configurations_by_client"
| extend client_id = tostring(customDimensions.client_id)
...
```kql
customMetrics
| where name == "file_processing_execution_failures_total"
| extend protocol = tostring(customDimensions.protocol)
...
2. Click "Upload" → Select `dashboards/file-processing-monitoring.json`
