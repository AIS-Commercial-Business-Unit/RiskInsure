# Operational Runbook: File Processing Configuration

**Purpose**: Troubleshooting guide for common operational scenarios in the File Processing system  
...
### Symptom
A FileProcessingConfiguration is marked as `IsActive = true` but no scheduled executions are occurring.
...
| Cause | Resolution |
|-------|------------|
| **Worker service not running** | Check Container App status, restart if needed: `az containerapp restart --name file-processing-worker --resource-group riskinsure-prod` |
...
1. If Worker not running:
   ```bash
   az containerapp restart --name file-processing-worker --resource-group riskinsure-prod
   ```
...
3. Monitor next execution:
   ...
   az containerapp logs show --name file-processing-worker --resource-group riskinsure-prod --follow
   ```
...
### Symptom
FileProcessingExecution records show `Status = Failed` with `ErrorCategory = "ConnectionFailure"` or `"ConnectionTimeout"`.
...
| **Azure Blob** | Managed identity not assigned | Assign identity: `az containerapp identity assign --name file-processing-worker --resource-group riskinsure-prod` |
...
2. **Check Worker resource utilization**:
   ...
   az monitor metrics list \
     --resource "/subscriptions/{sub}/resourceGroups/riskinsure-prod/providers/Microsoft.App/containerApps/file-processing-worker" \
...
|-------|-------|------------|
| **Liveness** | App crashed or deadlocked | Restart Container App: `az containerapp restart --name file-processing-api --resource-group riskinsure-prod` |
...
- **Architecture**: [services/file-processing/docs/file-processing-standards.md](file-processing-standards.md)
- **Deployment Guide**: [services/file-processing/docs/deployment.md](deployment.md)
- **Monitoring Dashboard**: [services/file-processing/docs/monitoring.md](monitoring.md)
- **Quickstart Guide**: [specs/001-file-processing-config/quickstart.md](../../specs/001-file-processing-config/quickstart.md)
