...
**File**: `src/FileProcessing.Infrastructure/Cosmos/CosmosEncryptionConfiguration.cs`
...
```json
{
  "CosmosDb": {
    ...
    "FileProcessingConfigsEncryptionKeyName": "file-processing-dek"
  },
  "AzureKeyVault": {
...
**Files Updated**:
- `src/FileProcessing.Infrastructure/Cosmos/CosmosDbContext.cs` (major updates)
- `src/FileProcessing.Infrastructure/DependencyInjection.cs` (added service registration)
...
3. **Key Design Decisions**:
   - Encryption policy is applied only to `file-processing-configurations` container (where protocol settings are stored)
...
2. **Create Data Encryption Key (DEK)**:
  ...
  - Key name is required in configuration (`CosmosDb:FileProcessingConfigsEncryptionKeyName`)
...
**Error**: "CosmosDb:FileProcessingConfigsEncryptionKeyName configuration is required"
...
### Created
- ✅ `src/FileProcessing.Infrastructure/Cosmos/CosmosEncryptionConfiguration.cs` (141 lines)
- ✅ `appsettings.encryption.example.json` (example configuration)

### Modified
- ✅ `src/FileProcessing.Infrastructure/Cosmos/CosmosDbContext.cs` (added encryption initialization)
- ✅ `src/FileProcessing.Infrastructure/DependencyInjection.cs` (added service registration)
