# Cosmos DB Setup Script for File Processing Service
...
param(
    ...
    [string]$DatabaseName = "file-processing"
)

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "  Cosmos DB Setup - File Processing Service" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
2. Create 'file-processing-configurations' container:
   az cosmosdb sql container create \
     --account-name $accountName \
     --database-name $DatabaseName \
     --name file-processing-configurations \
...
3. Create 'file-processing-executions' container:
   az cosmosdb sql container create \
     --account-name $accountName \
     --database-name $DatabaseName \
     --name file-processing-executions \
     --partition-key-path "/clientId" \
