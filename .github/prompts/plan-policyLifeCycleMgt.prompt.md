...
4. Update infrastructure wiring: NServiceBus endpoint names from "RiskInsure.Policy.*" to "RiskInsure.PolicyLifeCycleMgt.*", routing configurations, appsettings keys, and Cosmos settings for a fresh container (rename container from "policy" to "policylifecycle" + partition key strategy from `/policyId` to `/lifeCycleId`). *depends on step 2*
...
7. Update test structure: rename test namespaces (PolicyManagerTests → LifeCycleManagerTests), update Playwright config to new API port, update test data and assertions to use new terminology (policy → lifecycle, policyId → lifeCycleId), configure Integration.Tests to point to new API endpoint. *depends on step 5*
...
3. Start Cosmos DB Emulator: verify new container "policylifecycle" is created automatically with partition key "/lifeCycleId"
...
12. Verify logging: check logs contain new correlation identifiers (lifeCycleId), proper log levels (Information/Warning/Error), structured logging format, and sequential number generation audit trail
...
- Change partition key from `/policyId` to `/lifeCycleId` for domain clarity
...
1. **Partition Key Naming**: Change from `/policyId` to `/lifeCycleId` for domain clarity OR keep `/policyId` for consistency? Recommendation: Change to `/lifeCycleId` to fully embrace new domain model and avoid confusion.
